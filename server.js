 ...
 * @return {Object} forecast object.
 */
function generateFakeEvent(eventid) {
  eventid = eventid || 1;
  // Create a new copy of the event
  const result = Object.assign({}, fakeEvent);
  return result;
}
 ...
 * @param {Response} resp response object from Express.
 */
function getEvent(req, resp) {
  const event = req.params.id || 1;
  const url = `${BASE_URL}/${API_KEY}/${event}`;
  fetch(url).then((resp) => {
    if (resp.status !== 200) {
      throw new Error(resp.statusText);
    }
    ...
    console.error('Dark Sky API Error:', err.message);
    resp.json(generateFakeEvent(event));
  });
}
